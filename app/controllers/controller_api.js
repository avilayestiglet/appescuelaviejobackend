const controllerApi = {};
const { getUserLoginService, 
  getProfesoresService, getMateriasService, getUserService, getMatriculas, registerService, getAlumnosService, deleteProfesorIdService, deleteAlumnoID, getTipoUsuario, getMatriculasService, createProfesorService, registerBitacoraService, getBitacoraService, createMatriculaService } = require("../services/services");
...
controllerApi.loginApi = async (req, res, next) => {
    ...
    const result = await getUserLoginService({ user: { email: email, password: password } });
...
controllerApi.authApi = (req, res,next) => {
    ...
    if (token == null || token == undefined) {
      req.result = { status: 202, error: "Error al generar el token (token vacio o inválido)" }
      next();
      return;
    }
...
controllerApi.getProfesoresApi = async (req, res,next) => {
  ...
  const result = await getProfesoresService({filters: req.query});
  if (result.success){
...
controllerApi.createProfesorApi = async (req, res,next) => {
  ...
  if(errors.length != 0){
    req.result = { status: res.statusCode , error: 'Error al crear profesor', cause: errors }
    next();
    return;
  }
  ...
  if (result.success){
      if(result.error){
        req.result = { status: res.statusCode , error: result.error ?? 'Error al crear profesor', cause: errors }
    ...
    req.result ={
      status: 500,
      error: translate({x: result?.message ?? 'internal server error' ?? 'internal server error'}),
    }
    next();
    return; 
...
controllerApi.registerApi = async (req, res,next) => {
  ...
  if (result.success){
    ...
    if(!tipo_usuario){
      ...
      req.result = { status: res.statusCode, error: "El tipo de usuario no existe"}
      next();
      return; 
    ...
    if( !email || !password ){
      ...
      password == null || password == undefined ?  cause.push("clave_is_empty") : "";
    ...
    const resultRegister = await registerService({user: {email, password, id_tipo}});
    if(resultRegister.success){
      req.result = { status: res.statusCode , message: "Usuario registrado con éxito"}
...
controllerApi.createBitacoraApi = async (req, res) => {
  const resultBitacora = await registerBitacoraService({user: req.user, activity: req.activity ?? req.result?.error});
  return res.status(req.result.status).json(req.result);
}  
controllerApi.getBitacoraApi = async (req, res, next) => {
  const result = await getBitacoraService();
  if (result.success){
    req.result = { status: res.statusCode , data: result.data }
    req.activity = activity.bitacora('GET')
    next();
    return; 
  }else{
    req.result = {
